...
#     selected: --> boolean 
#      '''
#     def __init__(self, row, col, value = 0, editable = True):
#         self.value = value
#         self.row = row
#         self.col = col
...
#     def solution(self, solved_board):
#         ''' Update the userBoard with the solution board'''
#         if not self.solved:
#             self.userBoard = solved_board

#     def updateCell(self, val, row, col):
...
class Grid:
    ...
    def cellsStatus(self):
        ...
        # for i in range(9):
        #     for j in range(9):
        #         arr.append(self.cells[i][j].solved)
        # return arr
        arr = []
        for i in range(9):
            for j in range(9):
